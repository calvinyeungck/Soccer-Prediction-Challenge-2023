...
parser.add_argument('--year', type=str, default='19-20', help='year')
parser.add_argument('--d_ff', type=int, default=1,
                    help='the number of neurons in the feed-forward network') #[1,2,4]
args = parser.parse_args()

year=args.year
print(year,args.d_ff)
valid_set="exact"
...
df1=df.reset_index(drop=True) #reset index
df1=df1.drop(columns=('index')) #drop the column index
df1=df1.reset_index()
df1,Berrar_Recencyfeature_team_dict=features_create.Berrar_Recencyfeature_train(df1,n=5)
trainset=df1[:]
...
if __name__ == '__main__': 
    ...
    #model
    #for i in [0]:
    for inception_version in [1]: #fixed 1
        for d_ff in [4]: # fixed 4
            for embedding_dim in [1]: #fixed 1
                for TE_dim_feedforward in [1]:  #fixed 1, can't use 0  
                    for TE_dropout in [0]: #fixed 0
                        for encoder in encoder_list: #fixed "TE"
                            for num_layer in [10]:
                                for p in [0.2]:                                   
                                    LSTM_input_size=features_per_window+embedding_dim+embedding_dim
                                    LSTM_hidden_size=features_per_window+embedding_dim+embedding_dim
